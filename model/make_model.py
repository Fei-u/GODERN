...
def make_model(args):
    if args.model_type == 'fc':
        ...
        vector_field_2 = FinalTanh_f(input_channels=args.input_dim, hidden_channels=2*(args.hid_dim+5),
                                        hidden_hidden_channels=args.hid_hid_dim,
                                        num_hidden_layers=args.num_layers)
    else:
        if args.is_aug:
            ...
            vector_field_2 = VectorField_g(input_channels=args.input_dim, hidden_channels=2*(args.hid_dim+5),
                                            hidden_hidden_channels=args.hid_hid_dim,
                                            num_hidden_layers=args.num_layers, num_nodes=args.num_nodes, cheb_k=args.cheb_k, embed_dim=args.embed_dim, is_ode=args.is_ode)
        else:
